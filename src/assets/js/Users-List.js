...
export default class UsersList {
     ...
     */
    constructor(data = [], itemClickCallback = () => null, getUser = () => {}, config) {
     ...
     */
    _clickHandler(event) {
        const target = event.target.closest('.' + this.config.classNames.userItemContainerClass);
    ...
    _createPersonNode(userId, num) {
        ...
        el.classList.add(this.config.classNames.userItemContainerClass, 'clearfix');
        ...
        if (this.config.showAvatars) {
            el.querySelector('.' + this.config.classNames.userImageContainerClass).appendChild(this._renderPersonImage(user.pic));
        }
        return el;
    }
    ...
    _renderPerson(user) {
        return `
            <div class="user-item">
                ${this.config.showAvatars ? '<div class="user-item__image-container"></div>' : ''}
    ...
    _renderPersonImage(pic) {
        ...
        img.addEventListener('error', () => img.classList.add('picture-load-failure'));
