...
export default class DropdownController {
    ...
    init() {
        ...
        this.dataModel.data = [...this.dataModel.initialData].splice(0, 12).map(item => item.id);
        this.dataModel.state.list = [...this.dataModel.data].sort();
        this.usersList = new UsersList(this.dataModel.data, (id) => this._onItemSelect(id), (id) => this.dataModel.getUserById(id), this.config);
        this.selectedItems = new SelectedItems(this.element, (id) => this._removeSelectedItem(id), () => this.showUserList(),(id) => this.dataModel.getUserById(id), this.config);
    ...
    renderUsersList() {
        ...
        this.usersListElement.appendChild(this.usersList.render(this.dataModel.state.list));
    }
    ...
    _onChange(e) {
        ...
        if (value.length > 0) {
            if (this.config.fetchFromServer) {
                ...
                .then(res => {
                    this._setUserListData(this.dataModel.compareDataState(res, this.dataModel.state.selected));
                    this.renderUsersList();
                    return res;
                })
            ...
            this._setUserListData(this.dataModel.compareDataState(this.dataModel.data, this.dataModel.state.selected));
            this.renderUsersList();
        }
        ...
        const isIdFromUiData = this.dataModel.data.some(item => {
            if (item === id) {
                if (this.config.isMulti) {
                    ...
                    if (this.dataModel.state.selected.length) {
                        this.dataModel.state.list.push(this.dataModel.state.selected[0]);
    ...
    _removeSelectedItem(id) {
        ...
        if (index >= 0) {
            this.dataModel.state.list.push(this.dataModel.state.selected.splice(index, 1)[0]);
            this.dataModel.state.list.sort();
            this.renderUsersList();
            this.renderSelectedItems();
        }
    ...
    fetchData(value) {
        return new Promise(
            (resolve, reject) => {
              setTimeout(() => resolve(this.trie.getUserIds(value)), Math.random() * 100);
            }
          )
    }
     ...
     */
    sortData(value) {
        this._setUserListData(this.dataModel.compareDataState(this.trie.getUserIds(value), this.dataModel.state.selected));
    }
